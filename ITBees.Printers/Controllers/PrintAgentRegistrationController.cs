...
/// loopback callback. The agent then exchanges the code for its own token on the agent port.
/// </summary>
[Authorize]
public class PrintAgentRegistrationController : RestfulControllerBase<PrintAgentRegistrationController>
{
    ...
    {
        return ReturnOkResult(() => _registrationService.Create(registrationIm, Request.Scheme, Request.Host.Host));
    }
}
